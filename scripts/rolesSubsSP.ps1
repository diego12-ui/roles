...
Write-Host "Verificando si el sp ya cuenta con el rol en la suscripcion"
...
if ($Assigned) {
   Write-Host "El service principal '$sp' ya tiene el rol '$rol' asignado a nivel de suscripción."
}
...
New-AzRoleAssignment -ObjectId $idsp -RoleDefinitionName $rol -Scope "/subscriptions/$IdSuscripcion"

Write-Host "Se agrego el rol '$rol' permanete correspondiente al service principal '$sp' en la suscripcion '$suscripcion'"
