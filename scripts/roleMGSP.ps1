...
if ($ambiente -ne "Producción") {
   throw "El ambiente debe ser productivo cuando seleccionamos la suscripción."
}
...
Write-Host "Validar que el sp corresponda a 'PROD'."
if ($sp -notmatch "PRO") {
    throw "El service principal '$sp' no es de producción. Favor de ingresar correctamente el Service Principal" 
}
...
Write-Host "Verificando si el sp ya cuenta con el rol en el Management Group"

$assignments = Get-AzRoleAssignment -ObjectId $idsp -Scope "/providers/Microsoft.Management/managementGroups/$mg"
$Assigned = $assignments | Where-Object { $_.RoleDefinitionName -eq $rol }
if ($Assigned) {
   Write-Host "El Service Principal '$sp' ya tiene el rol '$rol' asignado a nivel de Management Group."
}

if ($tipoAsignacion -eq "Permanente") {
#Asignar rol para la  suscripçion
Write-Host "Asignando rol en el MG"
New-AzRoleAssignment -ObjectId $idsp -RoleDefinitionName $rol -Scope "/providers/Microsoft.Management/managementGroups/$mg"

Write-Host "Se agrego el rol '$rol' permanente correspondiente al Service Principal '$sp' en el management group '$mg'"
...
New-AzRoleAssignmentScheduleRequest -Name $guid -Scope "/providers/Microsoft.Management/managementGroups/$mg" -ExpirationType AfterDateTime -PrincipalId $idsp -RequestType AdminAssign -RoleDefinitionId "/providers/Microsoft.Authorization/roleDefinitions/$rolId" -ScheduleInfoStartDateTime $startTime -ExpirationEndDateTime  $endTime -Justification $Justification 

Write-Host "Se agrego el rol '$rol' correspondiente al Service Principal '$sp' en el management group '$mg'"
